...
export default function ProductList({ ActionButton }: ProductListProps) {
    ...
    const filteredProducts = useMemo(() => {
        ...
        // The SearchBar component uses 'query', so we must filter by 'query'
        const searchTerm = searchParams.get('query')?.toLowerCase() || '';
        const activeTags = searchParams.getAll('tags');

        // Filtering logic based on the 'query' URL parameter
        if (searchTerm) {
        ...
        // Additional filtering based on tags, which is applied after the search filter
